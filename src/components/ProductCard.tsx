      ...
      <div className="flex flex-1 flex-col gap-4 p-5">
        ...
        {highlights.length > 0 && (
          <dl className="grid grid-cols-1 gap-2 text-xs text-slate-500 dark:text-slate-400">
            {highlights.map(([label, value]) => (
              <div key={label} className="flex items-center justify-between rounded-lg bg-slate-50 px-3 py-2 dark:bg-slate-800/70">
                <dt className="font-medium capitalize text-slate-600 dark:text-slate-300">{label}</dt>
                <dd>{value}</dd>
              </div>
            ))}
          </dl>
        ...
        <div className="mt-auto flex items-end justify-between">
          <div>
            ...
            <p className="text-2xl font-semibold text-slate-900 dark:text-slate-100">{formatPrice(product.price)}</p>
          </div>
          <button
