  ...
  return (
    <section className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
      <header className="mb-10 space-y-3">
        <p className="text-sm font-semibold uppercase tracking-wide text-slate-500">Categoria</p>
        <h1 className="text-4xl font-semibold text-slate-900">
          {currentCategory?.name ?? 'Componentes'}
        </h1>
        {currentCategory?.description && (
          <p className="text-sm text-slate-500 max-w-2xl">{currentCategory.description}</p>
        )}
      </header>
      ...
      {loading ? (
        ...
        <div className="flex min-h-[240px] items-center justify-center rounded-3xl border border-dashed border-slate-200 bg-white">
          <p className="text-sm text-slate-500">No hay productos disponibles en esta categoria ): </p>
        </div>
      ) : (
